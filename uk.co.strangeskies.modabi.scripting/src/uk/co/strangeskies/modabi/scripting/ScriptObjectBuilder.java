...
public class ScriptObjectBuilder<T> {
  ...
  public ScriptObject<T> create(ClassLoader classLoader) throws ScriptException {
    Class<?> rawType = objectType.getErasedType();

    Property<ScriptObject<T>> proxy = new IdentityProperty<>();
    ...
    ScriptObject<T> scriptObject = new ScriptObject<T>() {
      TypeToken<ScriptObject<T>> objectType = getScriptObjectType(
          ScriptObjectBuilder.this.objectType);
      ...
      @Override
      public TypeToken<ScriptObject<T>> getThisTypeToken() {
        return objectType;
      }

      private <U> TypeToken<ScriptObject<U>> getScriptObjectType(TypeToken<U> objectType) {
        return new TypeToken<ScriptObject<U>>() {}
            .withTypeArguments(new TypeArgument<U>(objectType) {});
      }
    ...
    proxy
        .set(
            (ScriptObject<T>) Proxy
                .newProxyInstance(
                    classLoader,
                    new Class[] { rawType, ScriptObject.class },
