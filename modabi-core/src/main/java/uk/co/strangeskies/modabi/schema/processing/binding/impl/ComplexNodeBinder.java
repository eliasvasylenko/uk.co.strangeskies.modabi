...
public class ComplexNodeBinder {
	...
	public <U> List<U> bind2(ComplexNode.Effective<U> node) {
		...
		do {
			...
			if (node.isInline()) {
				...
				if (!context.input().peekNextChild().equals(node.getName()))
					break;
	...
	public <U> List<U> bind(ComplexNode.Effective<U> node) {
		...
		int count = 0;
		ComplexNode.Effective<U> inputNode;
		do {
			inputNode = null;
			...
			if (nextElement != null) {
				if (node.isExtensible()) {
					...
					inputNode = (ComplexNode.Effective<U>) attemptedOverrideMap
							.get(nextElement);
					if (inputNode == null) {
						inputNode = new ComplexNodeOverrider(
								context.provide(ModelBuilder.class)).override(node,
								(Model.Effective<U>) extension.effective());
