...
public interface BindingBlocks extends Observable<BindingBlockEvent> {
  BindingBlocks NON_BLOCKING = new NonBlockingBindingBlocks();
  ...
  /**
   * Wait until no blocks remain held in the binding system. This may not signify
   * the end of binding, or that more blocks will not occur.
   * 
   ...
   */
  void waitForAll(long timeoutMilliseconds)
      throws InterruptedException, TimeoutException, ExecutionException;

  /**
   * Determine whether the binding block system as a whole is blocked. It is
   * considered blocked if every participating {@link Thread} registered with the
   * associated {@link BindingBlocker} is waiting on a block via
   ...
   * 
   * @return True if the binding is blocked and no processing threads are active,
   *         false otherwise
   */
  boolean isBlocked();
...
class NonBlockingBindingBlocks implements BindingBlocks, ImmutableObservable<BindingBlockEvent> {
  @Override
