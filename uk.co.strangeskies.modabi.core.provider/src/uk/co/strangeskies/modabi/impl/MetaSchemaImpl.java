...
public class MetaSchemaImpl implements MetaSchema {
	...
	public MetaSchemaImpl(SchemaBuilder schema, ModelBuilder modelBuilder, DataTypeBuilder dataTypeBuilder,
			DataLoader loader, BaseSchema base) {
		...
		Model<ChildNode<?, ?>> childModel = factory.apply("child",
				m -> m.baseModel(branchModel, childBaseModel).isAbstract(true).dataType(new TypeToken<ChildNode<?, ?>>() {})
						...
						.addChild(n -> n.data().format(Format.PROPERTY).name("occurrences").type(base.derivedTypes().rangeType())
								.optional(true))
						.addChild(n -> n.data().format(Format.PROPERTY).type(base.derivedTypes().typeTokenType())
								...
								.addChild(
										n -> n.data().format(Format.PROPERTY)
												...
												.type(
														base.derivedTypes()
																.listType())
												.outMethodUnchecked(true).inMethodUnchecked(true)
												.addChild(o -> o.data().name("element").type(base.derivedTypes().referenceType())
														...
														.addChild(p -> p.data().name("isExternal")
																.provideValue(new BufferingDataTarget().put(Primitive.BOOLEAN, false).buffer())))));

		metaModel = factory.apply("model",
				m -> m.baseModel(abstractModelModel).dataType(new TypeToken<Model<?>>() {}).bindingType(ModelBuilder.class)
						...
						m -> m.baseModel(bindingNodeModel).dataType(new TypeToken<DataType<?>>() {})
								...
								.addChild(
										c -> c.inputSequence().name("configure").inMethodChained(true)
												.addChild(
														d -> d.data().dataType(DataLoader.class).bindingStrategy(BindingStrategy.PROVIDED)
		...
		schemaModel = factory.apply("schema",
				m -> m.dataType(Schema.class).bindingType(SchemaBuilder.class)
						...
						.addChild(n -> n.data().format(Format.PROPERTY).name("name").inMethod("qualifiedName")
								.outMethod("getQualifiedName").type(
										base.primitiveType(
								...
								i -> i.data().format(Format.SIMPLE).name("imports").optional(true)
										...
										.addChild(
												n -> n.data().name("importsIn").inMethod("imports").outMethod("null")
														.type(base.derivedTypes().setType()).addChild(e -> e.data().name("element")
																.type(base.derivedTypes().classType()).occurrences(Range.between(1, 1))))
										.addChild(
												n -> n.data().name("importsOut").inMethod("null")
														.optional(
																true)
														.outMethod("getImports").dataType(Imports.class).addChild(
								...
								.addChild(o -> o.data().inMethod("add").name("element").type(base.derivedTypes().importType())
										...
										.addChild(p -> p.data().name("models").inMethod("null").type(base.derivedTypes().includeType())
												...
												.addChild(q -> q.data().name("targetModel")
														.provideValue(new BufferingDataTarget()
																.put(Primitive.QUALIFIED_NAME, new QualifiedName("model", namespace)).buffer())))))
						.addChild(n -> n.complex().name("types").outMethod("getDataTypes").occurrences(Range.between(0, 1))
								.dataType(new @Infer TypeToken<Set<?>>() {}).bindingType(new @Infer TypeToken<LinkedHashSet<?>>() {})
								.addChild(o -> o.complex().baseModel(dataTypeModel).outMethod("this").name("type")
										.dataType(new TypeToken<DataType<?>>() {}).occurrences(Range.between(0, null))))
						.addChild(n -> n.complex().name("models").occurrences(Range.between(0, 1))
								.dataType(new @Infer TypeToken<Set<?>>() {}).bindingType(new @Infer TypeToken<LinkedHashSet<?>>() {})
								.addChild(o -> o.complex().baseModel(metaModel).inMethod("add").outMethod("this")
