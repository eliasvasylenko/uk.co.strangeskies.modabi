...
public class MetaSchemaImpl implements MetaSchema {
	...
	private void buildModels(ModelFactory factory, BaseSchema base, Namespace namespace) {
		...
		schemaModel = factory.apply("schema", m -> m.dataType(Schema.class).bindingType(SchemaBuilder.class)
				...
				.addChild(
						n -> n.data().format(Format.PROPERTY).name("name").inMethod("qualifiedName").outMethod("getQualifiedName")
								.type(
						...
						n -> n.data().format(Format.SIMPLE).name("dependencies")
								...
								.addChild(
										o -> o.data().inMethod("add").name("element").type(base.derivedTypes().importType())
												.dataType(Schema.class).outMethod("this")
												.occurrences(
														Range
																.between(0,
																		null))
				...
				.addChild(
						n -> n.complex().name("types").outMethod("this").occurrences(Range.between(0, 1)).dataType(Schema.class)
								...
								.addChild(o -> o.complex().baseModel(dataTypeModel).inMethod("null")
				...
				.addChild(
						n -> n.complex().name("models").outMethod("this").occurrences(Range.between(0, 1)).dataType(Schema.class)
								.bindingType(SchemaConfigurator.class).inMethod("null").bindingStrategy(BindingStrategy.TARGET_ADAPTOR)
								.addChild(o -> o.complex().baseModel(metaModel).inMethod("null")
										.bindingStrategy(BindingStrategy.TARGET_ADAPTOR).outMethod("getModels")
