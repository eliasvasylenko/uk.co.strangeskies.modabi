...
public class MetaSchemaImpl implements MetaSchema {
	...
	public MetaSchemaImpl(SchemaBuilder schema, ModelBuilder model, DataBindingTypeBuilder dataType, DataLoader loader,
			BaseSchema base) {
		...
		Model<SchemaNode<?, ?>> nodeModel = model.configure(loader).name("node", namespace).isAbstract(true)
				.dataType(new TypeToken<SchemaNode<?, ?>>() {}).unbindingStrategy(UnbindingStrategy.SIMPLE)
				.addChild(n -> n.inputSequence().name("configure").isAbstract(true)
						.postInputType(new TypeToken<SchemaNodeConfigurator<?, ?>>() {}))
				.addChild(n -> n.data().format(Format.PROPERTY).type(base.primitiveType(DataType.QUALIFIED_NAME)).name("name")
						.inMethod("name").optional(true))
				.addChild(n -> n.data().format(Format.PROPERTY).type(base.primitiveType(DataType.BOOLEAN)).name("abstract")
						.inMethod("isAbstract").optional(true))
				.create();
		modelSet.add(nodeModel);

		Model<ChildNode<?, ?>> childBaseModel = model.configure(loader).name("childBase", namespace).isAbstract(true)
				.dataType(new TypeToken<ChildNode<?, ?>>() {}).bindingType(new TypeToken<SchemaNodeConfigurator<?, ?>>() {})
				.create();
		modelSet.add(childBaseModel);

		Model<SchemaNode<?, ?>> branchModel = model.configure(loader).name("branch", namespace).isAbstract(true)
				.baseModel(nodeModel).addChild(n -> n.data().name("name")).addChild(n -> n.data().name("abstract"))
				.addChild(n -> n.complex().name("child").outMethod("children").inMethod("null").isAbstract(true)
						.extensible(true).baseModel(childBaseModel).outMethodIterable(true).occurrences(Range.create(0, null)))
				.addChild(n -> n.inputSequence().name("create").inMethodChained(true)).create();
		modelSet.add(branchModel);

		Model<ChildNode<?, ?>> childModel = model.configure(loader).name("child", namespace)
				.baseModel(branchModel, childBaseModel).isAbstract(true).dataType(new TypeToken<ChildNode<?, ?>>() {})
				...
				.addChild(n -> n.data().name("name")).addChild(n -> n.data().format(Format.PROPERTY)
						.type(base.derivedTypes().typeTokenType()).name("postInputType").optional(true))
				.create();
		modelSet.add(childModel);

		Model<BindingNode<?, ?, ?>> bindingNodeModel = model.configure(loader).name("binding", namespace)
				.baseModel(branchModel).isAbstract(true).dataType(new TypeToken<BindingNode<?, ?, ?>>() {})
				...
				.addChild(n -> n.data().format(Format.PROPERTY).name("providedUnbindingMethodParameters").optional(true)
						...
						.addChild(o -> o.data().name("element").type(base.primitiveType(DataType.QUALIFIED_NAME))))
				.addChild(n -> n.data().format(Format.PROPERTY).name("unbindingFactoryType").optional(true)
						.type(base.derivedTypes().typeTokenType()))
				.create();
		modelSet.add(bindingNodeModel);

		Model<InputNode<?, ?>> inputModel = model.configure(loader).name("input", namespace).isAbstract(true)
				.baseModel(childModel).dataType(new TypeToken<InputNode<?, ?>>() {})
				.addChild(c -> c.inputSequence().name("configure").isAbstract(true)
		...
		Model<BindingChildNode<?, ?, ?>> bindingChildNodeModel = model.configure(loader).name("bindingChild", namespace)
				.isAbstract(true).dataType(new TypeToken<BindingChildNode<?, ?, ?>>() {})
				.baseModel(inputModel, bindingNodeModel)
				.addChild(c -> c.inputSequence().name("configure").isAbstract(true)
						.postInputType(new TypeToken<BindingChildNodeConfigurator<?, ?, Object>>() {}))
				.addChild(n -> n.data().name("name"))
				.addChild(n -> n.data().format(Format.PROPERTY).name("extensible").type(base.primitiveType(DataType.BOOLEAN))
						.optional(true))
				.addChild(n -> n.data().format(Format.PROPERTY).name("ordered").type(base.primitiveType(DataType.BOOLEAN))
						.optional(true))
		...
		Model<ChoiceNode> choiceModel = model.configure(loader).name("choice", namespace).dataType(ChoiceNode.class)
				.baseModel(childModel).addChild(c -> c.inputSequence().name("configure").inMethod("choice"))
				.addChild(n -> n.data().name("name")).addChild(n -> n.data().format(Format.PROPERTY).name("mandatory")
						.optional(true).type(base.primitiveType(DataType.BOOLEAN)))
				.create();
		modelSet.add(choiceModel);

		Model<SequenceNode> sequenceModel = model.configure(loader).name("sequence", namespace).dataType(SequenceNode.class)
				.baseModel(childModel).addChild(c -> c.inputSequence().name("configure").inMethod("sequence")).create();
		modelSet.add(sequenceModel);

		Model<InputSequenceNode> inputSequenceModel = model.configure(loader).name("inputSequence", namespace)
				.dataType(InputSequenceNode.class).baseModel(inputModel, childModel).addChild(c -> c.inputSequence()
						.name("configure").inMethod("inputSequence").postInputType(InputSequenceNodeConfigurator.class))
				.addChild(n -> n.data().name("name")).create();
		modelSet.add(inputSequenceModel);

		Model<AbstractComplexNode<?, ?, ?>> abstractModelModel = model.configure(loader).name("abstractModel", namespace)
				.baseModel(
						bindingNodeModel)
				.isAbstract(true)
				.dataType(
				...
				.addChild(
						n -> n.data().format(Format.PROPERTY).name("baseModel")
								.inMethod(
										"baseModel2")
								.optional(true).type(base.derivedTypes().listType()).outMethodUnchecked(true).inMethodUnchecked(true)
								.addChild(o -> o.data().name("element").type(base.derivedTypes().referenceType())
										...
										.addChild(p -> p.data().name("targetModel").dataType(new TypeToken<Model<Model<?>>>() {})
												.provideValue(new BufferingDataTarget()
														.put(DataType.QUALIFIED_NAME, new QualifiedName("model", namespace)).buffer()))
										.addChild(
												p -> p.data().name("targetId")
														.provideValue(new BufferingDataTarget()
																.put(DataType.QUALIFIED_NAME, new QualifiedName("name", namespace)).buffer()))))
				.create();
		modelSet.add(abstractModelModel);

		Model<Model<?>> modelModel = model.configure(loader).name("model", namespace).baseModel(abstractModelModel)
				.dataType(new TypeToken<Model<?>>() {}).bindingType(ModelBuilder.class)
				.addChild(c -> c.inputSequence().name("configure")
						.inMethodChained(true).addChild(d -> d.data().dataType(DataLoader.class)
								.bindingStrategy(BindingStrategy.PROVIDED).name("configure").outMethod("null")))
				.addChild(n -> n.data().name("name").optional(false)).create();
		modelSet.add(modelModel);

		Model<ComplexNode<?>> abstractComplexModel = model.configure(loader).name("abstractComplex", namespace)
				.isAbstract(true).dataType(new TypeToken<ComplexNode<?>>() {})
				...
				.addChild(n -> n.data().name("name")).addChild(c -> c.data().name("inline").isAbstract(true).optional(true)
						.valueResolution(ValueResolution.REGISTRATION_TIME).type(base.primitiveType(DataType.BOOLEAN)))
				.create();
		modelSet.add(abstractComplexModel);

		Model<ComplexNode<?>> complexModel = model.configure(loader).name("complex", namespace)
				.baseModel(abstractComplexModel).addChild(c -> c.data().name("inline").optional(true)
						.provideValue(new BufferingDataTarget().put(DataType.BOOLEAN, false).buffer()))
				.create();
		modelSet.add(complexModel);

		Model<ComplexNode<?>> inlineModel = model.configure(loader).name("inline", namespace)
				.baseModel(abstractComplexModel).addChild(c -> c.data().name("inline").optional(false)
						.provideValue(new BufferingDataTarget().put(DataType.BOOLEAN, true).buffer()))
				.create();
		modelSet.add(inlineModel);

		Model<DataNode<?>> typedDataModel = model.configure(loader).baseModel(bindingChildNodeModel)
				.name("typedData", namespace).dataType(new TypeToken<DataNode<?>>() {}).isAbstract(true)
				...
				.addChild(n -> n.data().format(Format.PROPERTY).name("format").optional(true)
						.valueResolution(ValueResolution.REGISTRATION_TIME).isAbstract(true).type(base.derivedTypes().enumType())
				...
				.addChild(n -> n.data().format(Format.PROPERTY).name("type").optional(true)
						.type(base.derivedTypes().referenceType()).dataType(new TypeToken<DataBindingType<?>>() {})
						.addChild(p -> p.data().name("targetModel").valueResolution(ValueResolution.REGISTRATION_TIME)
								.provideValue(new BufferingDataTarget()
										.put(DataType.QUALIFIED_NAME, new QualifiedName("type", namespace)).buffer()))
						.addChild(p -> p.data().name("targetId").valueResolution(ValueResolution.REGISTRATION_TIME)
								.provideValue(new BufferingDataTarget()
										.put(DataType.QUALIFIED_NAME, new QualifiedName("name", namespace)).buffer())))
				.addChild(n -> n.data().format(Format.PROPERTY).name("optional").optional(true)
						.type(base.primitiveType(DataType.BOOLEAN)))
		...
		Model<DataNode<?>> contentModel = model.configure(loader).name("content", namespace).baseModel(typedDataModel)
				.addChild(n -> n.data().name("format").optional(false)
						.provideValue(new BufferingDataTarget().put(DataType.STRING, "CONTENT").buffer()))
				.create();
		modelSet.add(contentModel);

		Model<DataNode<?>> propertyModel = model
				.configure(loader).name("property", namespace).baseModel(typedDataModel).addChild(n -> n.data().name("format")
						.optional(false).provideValue(new BufferingDataTarget().put(DataType.STRING, "PROPERTY").buffer()))
				.create();
		modelSet.add(propertyModel);

		Model<DataNode<?>> simpleModel = model.configure(loader).name("simple", namespace).baseModel(typedDataModel)
				.addChild(n -> n.data().name("format").optional(false)
						.provideValue(new BufferingDataTarget().put(DataType.STRING, "SIMPLE").buffer()))
				.create();
		modelSet.add(simpleModel);

		Model<DataNode<?>> dataModel = model.configure(loader).name("data", namespace).baseModel(typedDataModel)
				.addChild(n -> n.data().name("format").occurrences(Range.create(0, 0))).create();
		modelSet.add(dataModel);

		/* Type Models */

		Model<DataBindingType<?>> typeModel = model.configure(loader).baseModel(bindingNodeModel).name("type", namespace)
				.dataType(new TypeToken<DataBindingType<?>>() {}).bindingType(DataBindingTypeBuilder.class)
				...
				.addChild(n -> n.data().format(Format.PROPERTY).name("private").inMethod("isPrivate").optional(true)
						.type(base.primitiveType(DataType.BOOLEAN)))
				.addChild(n -> n.data().format(Format.PROPERTY).name("baseType").optional(true)
						...
						.addChild(p -> p.data().name("targetModel")
								.provideValue(new BufferingDataTarget()
										.put(DataType.QUALIFIED_NAME, new QualifiedName("type", namespace)).buffer()))
						.addChild(p -> p.data().name("targetId").provideValue(
								new BufferingDataTarget().put(DataType.QUALIFIED_NAME, new QualifiedName("name", namespace)).buffer())))
				.create();
		modelSet.add(typeModel);

		/* Schema Models */

		schemaModel = model.configure(loader).name("schema", namespace).dataType(Schema.class)
								...
								.occurrences(
										Range
												.create(0,
														1))
								.dataType(
										new TypeToken<Set<Schema>>() {})
								.addChild(
										o -> o.data().format(Format.SIMPLE).inMethod("add").name("dependency")
												...
												.addChild(i -> i.data().name("import")
														.addChild(p -> p.data().name("targetModel")
																.provideValue(new BufferingDataTarget()
																		.put(DataType.QUALIFIED_NAME, new QualifiedName("schema", namespace)).buffer()))
												...
												.addChild(p -> p.data().name("models").inMethod("null").type(base.derivedTypes().includeType())
														...
														.addChild(q -> q.inputSequence().name("getModels").inMethodChained(true))
														.addChild(q -> q.data().name("targetModel")
																.provideValue(new BufferingDataTarget()
																		.put(DataType.QUALIFIED_NAME, new QualifiedName("model", namespace)).buffer())))))
				.addChild(n -> n.complex().name("types").outMethod("getDataTypes").occurrences(Range.create(0, 1))
						.dataType(new TypeToken<@Infer Set<?>>() {}).bindingType(new TypeToken<@Infer LinkedHashSet<?>>() {})
						.addChild(o -> o.complex().baseModel(typeModel).outMethod("this").name("type").outMethodIterable(true)
								.dataType(new TypeToken<DataBindingType<?>>() {}).occurrences(Range.create(0, null))))
				.addChild(n -> n.complex().name("models").occurrences(Range.create(0, 1))
						.dataType(new TypeToken<@Infer Set<?>>() {}).bindingType(new TypeToken<@Infer LinkedHashSet<?>>() {})
						.addChild(o -> o.complex().baseModel(modelModel).inMethod("add").outMethodIterable(true).outMethod("this")
		 ...
		 * Schema
		 */
		metaSchema = schema.configure().qualifiedName(name).dependencies(Arrays.asList(base)).types(typeSet)
