...
public class BindingFutureImpl<T> implements BindingFuture<T> {
  ...
  public static class BindingSource<T> {
    private final BindingPoint<? super T> bindingPoint;
    ...
    public BindingSource(
        BindingPoint<? super T> bindingPoint,
        Model<? super T> model,
  ...
  private final FutureTask<Binding<T>> dataFuture;
  ...
  public BindingFutureImpl(
    ...
    dataFuture = new FutureTask<>(() -> {
      ...
      Binding<T> binding = bind(sourceFuture.get());
  ...
  private Binding<T> tryGet(TryGet<BindingSource<T>> getModel, TryGet<Binding<T>> getData) {
    synchronized (context.bindingBlocker()) {
    ...
    try {
      ...
      Model<? super T> model = source.getModel();

      modelString = " with model '" + model.name() + "'";

      input = source.getName();

      bindingResult = getData.tryGet();
  ...
  private <U> TypeToken<Model<U>> getModelType(TypeToken<U> type) {
    return new TypeToken<Model<U>>() {}.withTypeArguments(new TypeArgument<U>(type) {});
  }

  private <U> Binding<Model<U>> getModelBinding(Model<U> schemaModel) {
    return Binding.anonymous(
        context.manager().getMetaSchema().getMetaModel(),
        getModelType(schemaModel.dataType()),
        schemaModel);
  }

  private Binding<T> bind(BindingSource<T> source) {
    BindingPoint<? super T> bindingPoint = source.getBindingPoint();
    Model<? super T> model = source.getModel();
    ...
    return source.withData((StructuredDataReader input) -> {
       ...
       * associated model.
       */
      model.schema().models().forEach(schemaModel -> {
        context.bindings().add(getModelBinding(schemaModel));
      });

      QualifiedName inputRoot = input.getNextChild().get();
