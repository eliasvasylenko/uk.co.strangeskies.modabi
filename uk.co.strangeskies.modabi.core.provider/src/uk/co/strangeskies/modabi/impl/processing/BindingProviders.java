...
public class BindingProviders {
  ...
  private static class DereferenceImportBindingProvider implements ModelBindingProvider {
    ...
    public <T> Set<T> getAndListen(ComplexNode<T> model, Function<? super T, Boolean> listener) {
      ObservableSet<?, ModelBinding<T>> bindings = context.manager().getBindings(model);
  ...
  public Function<ProcessingContext, DataLoader> dataLoader() {
    return context -> new DataLoader() {
      ...
      public <U> List<U> loadData(SimpleNode<U> node, DataSource data) {
        return new DataNodeBinder<>(context, node)
            .getBinding()
            .stream()
            .map(ChildNodeBinding::getData)
            .collect(toList());
      }
    };
  }
  ...
  public Function<ProcessingContext, SchemaConfigurator> schemaConfigurator() {
    return context -> context.provide(SchemaBuilder.class).getObject().configure(
        context.provide(DataLoader.class).getObject());
  }
  ...
  public void registerProviders(Providers providers) {
    providers.add(Provider.over(DereferenceSource.class, dereferenceSource()));
    providers.add(Provider.over(ImportSource.class, importSource()));
    providers.add(Provider.over(DataLoader.class, dataLoader()));
    providers.add(Provider.over(SchemaConfigurator.class, schemaConfigurator()));
    providers.add(Provider.over(Imports.class, imports()));
  }
}
